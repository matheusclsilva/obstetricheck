...
export const Chip: React.FC<ChipProps> = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-medium border transition-all flex items-center gap-1.5 active:scale-95 text-left cursor-pointer ${
        disabled ? 'opacity-50 cursor-not-allowed' : ''
      } ${colorClasses[color] || colorClasses.teal}`}
    >
      {Icon && <Icon className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${active ? 'text-white' : 'text-slate-400'}`} />}
      <span>{label}</span>
      {badge && (
        <span
          className={`ml-1 text-[9px] sm:text-[10px] px-1.5 py-0.2 rounded-full font-bold uppercase ${
